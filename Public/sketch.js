...
let engine, world;
let genres = ['Horror', 'Romance', 'Sci-Fi', 'Documentary', 'Action', 'Musical', 'Thriller'];
let checkboxes = [];
let genreInstances = []; 
...
function setup() {
  ...
  let thickness = 50;
  let bottom = Bodies.rectangle(centerX, centerY + radius, radius * 2, thickness, { 
    isStatic: true 
  });
  let leftWall = Bodies.rectangle(centerX - radius, centerY + radius/2, thickness, radius * 1.5, { 
    isStatic: true, 
    angle: -QUARTER_PI 
  });
  let rightWall = Bodies.rectangle(centerX + radius, centerY + radius/2, thickness, radius * 1.5, { 
    isStatic: true, 
    angle: QUARTER_PI 
  });
...
function addTextBody(label) {
  ...
  let newBox = new Box(centerX + random(-20, 20), centerY - 150, label, world, col);
